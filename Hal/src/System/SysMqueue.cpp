 ...
 */
INT SysMqueue_create(MSG_Q_ID *pStHandle, const CHAR *strName, INT iMaxMsg, INT iMsgLen)
{
    CHAR sName[64] = {0};
    MSG_Q_ATTR stAttr = {0};
    MSG_Q_ID stMqId;
    ...
    {
        LOG_ERROR("Mqueue open invalid ptr.\n");
        return ERROR;
    }
    ...
    if(stMqId == ERROR)
    {
        LOG_ERROR("Mqueue %s open failed err:%s.\n", sName, strerror(errno));
        return ERROR;
    }
 ...
 */
INT SysMqueue_send(MSG_Q_ID *pStHandle, const CHAR *strMsg, UINT uLength, INT iWaitMs)
{
    INT iRet = 0;
    ...
    {
        LOG_ERROR("Mqueue send invalid ptr.\n");
        return ERROR;
    }
 ...
 * @return     成功返回 0  错误返回 其他, 参考ERROR_CODE_E
 */
INT SysMqueue_recv(MSG_Q_ID *pStHandle, CHAR *strBuff, INT iBuffLen, INT iWaitMs)
{
    INT iRet = 0;
 ...
 * @return     成功返回 0 错误返回 其他, 参考ERROR_CODE_E
 */
INT SysMqueue_close(MSG_Q_ID *pStHandle)
{   
    if(NULL == pStHandle)
    {
        return ERROR;
