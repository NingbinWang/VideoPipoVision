...
static UINT32 uevent_action_parse(CHAR *pBuf)
{
    if (0 == strcmp(pBuf, "add")) {
        ...
        LOG_WARNING("unknow action type(%s)\n", pBuf);
...
static UINT32 uevent_subsystem_parse(CHAR *pBuf)
{
    if (0 == strcmp(pBuf, "mmc")) {
        ...
        LOG_WARNING("unknow subsystem type(%s)\n", pBuf);
...
static UINT32 uevent_block_devtype_parse(CHAR *pBuf)
{
    if (0 == strcmp(pBuf, "disk")) {
        ...
        LOG_WARNING("unknow block devtype(%s)\n", pBuf);
...
static UINT32 uevent_usb_devtype_parse(CHAR *pBuf)
{
    if (0 == strcmp(pBuf, "usb_device")) {
        ...
        LOG_WARNING("unknow usb devtype(%s)\n", pBuf);
    ...
    switch (uMsgCnt) {
        ...
        case 3:
            if (0 == strncmp("SUBSYSTEM", pBuf, 9)) {
                ...
                LOG_ERROR("msg subsystem error: %s\n", pBuf);
            }
            return ;
        default:
            break;
...
INT32 SysUeventNetlink_recv(INT32 iSockfd, VOID *pBuffer, UINT32 uBufferLen)
{
    ...
    for (j = 0; i < iReplen && aRecvBuf[i]; j++) {
        LOG_DEBUG("recv uevent msg: %s\n", aRecvBuf + i);
        uevent_msg_handle(j, (SYS_UEVENT_NETLINK_RECV_T *)pBuffer, (CHAR *)(aRecvBuf + i), uBufferLen - sizeof(SYS_UEVENT_NETLINK_RECV_T));
