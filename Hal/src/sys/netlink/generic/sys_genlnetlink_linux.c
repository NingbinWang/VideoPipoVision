...
static INT32 genlnetlink_msg_check(GENCMSG_HEADER_T *pMsg, INT32 iReplen)
{
    ...
    if (pMsg->stNlh.nlmsg_type == NLMSG_ERROR || !NLMSG_OK((&pMsg->stNlh), iReplen)) {
        pErr = NLMSG_DATA(pMsg);
        PR_ERR("fatal reply error %d nlmsg_type(%u) nlmsg_len(%u) replen(%d)\n",
...
INT32 sys_genlnetlink_get_id(INT32 iSockfd, SYS_GENLNETLINK_ID_T *pGenlId)
{
    ...
    if (NULL == pGenlId) {
        PR_ERR("Invaild params pGenlId(%p)\n", pGenlId);
        return -EINVAL;
    } else {
        if (NULL == pGenlId->pFamily) {
            PR_ERR("Invaild params pFamily(%p)\n", pGenlId->pFamily);
            return -EINVAL;
        } 
    }
...
INT32 sys_genlnetlink_send_msg(INT32 iSockfd, VOID *pBuffer, UINT32 uBufferLen)
{
    ...
    if (pConfig->iFamilyId <= 0 || !pConfig->uAttrNr) {
        PR_ERR("invalid familyid(%d) attr num(%u)\n", pConfig->iFamilyId, pConfig->uAttrNr);
        return -EINVAL;
    }
    ...
    for (iLoopCnt = 0, pAttrConf = (SYS_GENLNETLINK_ATTR_T *)pConfig->aAttrMsg;
        ...
        if (0 == pAttrConf->uDataLen || uBufferLenTmp < (sizeof(SYS_GENLNETLINK_ATTR_T) + pAttrConf->uDataLen)) {
            PR_WRN("Invalid uDataLen(%u) uBufferLenTmp(%u)\n", pAttrConf->uDataLen, uBufferLenTmp);
            break;
        }
    ...
    pMsg = malloc(iMsgLen);
    if (!pMsg) {
        PR_ERR("malloc (%d)bytes failed errno[%d:%s]\n", iMsgLen, errno, strerror(errno));
        return -1;
    }
    ...
    while ((iRet = sendto(iSockfd, (CHAR *)pMsg, iMsgLen, 0, (struct sockaddr *) &stDstaddr, sizeof(stDstaddr))) < iMsgLen) {
        if (iRet > 0) {
            ...
            PR_ERR("genlmsg send failed - errno[%d:%s]\n", errno, strerror(errno));
            free(pMsg);
            return -1;
        }
    }
...
INT32 sys_genlnetlink_recv_msg(INT32 iSockfd, VOID *pBuffer, UINT32 uBufferLen)
{
    ...
    for (pAttrMsg = (SYS_GENLNETLINK_ATTR_T *)pConfig->aAttrMsg;
        ...
        if (uBufferLenTmp < sizeof(SYS_GENLNETLINK_ATTR_T) + NLA_ALIGN(pNla->nla_len - NLA_HDRLEN)) {
            PR_WRN("Buffer len is too small uBufferLenTmp(%u) attrlen(%u)\n", uBufferLenTmp, pNla->nla_len);
            break;
        }
