...
{    
    ...
    SYS_MEDIUM_FS_MAGIC_YAFFS2 = 0x5941ff53,
    SYS_MEDIUM_FS_MAGIC_EXFAT_NOFUSE = 0x2011bab0,
 ...
 */
INT32 sys_medium_fs_type_convert_data(SYS_MEDIUM_FS_TYPE_E eFsType,char *strData,UINT16 uSize)
{
    INT32 iRet = ERROR;
    if(!strData)
    {
    ...
    switch(eFsType)
    {
        ...
        case SYS_MEDIUM_FS_NTFS:
            /* do nothing */
            break;
        case SYS_MEDIUM_FS_YAFFS2:
            /* do nothing */
            break;
        default:
            PR_ERR("no support type :%d \n",eFsType);
            return iRet;
    }
 ...
 * @return 成果返回OK，失败返回ERROR,
 */
INT32 sys_medium_fs_type_convert_string(SYS_MEDIUM_FS_TYPE_E eFsType,char *strFileSystem,UINT16 uSize)
{   
    if(!strFileSystem)
    {
        return -1;
    ...
    switch(eFsType)
    {
        ...
        case SYS_MEDIUM_FS_EXFAT:
            ...
            break;
        case SYS_MEDIUM_FS_YAFFS2:
            strncpy(strFileSystem,"yaffs2",uSize);
            break;
        default:
            PR_ERR("no support type :%d \n",eFsType);
            return -1;
 ...
 * @return     成果返回OK，失败返回ERROR,
 */
INT32 sys_medium_mount_exfat(const char *strDevPath,const char *strMountPath)
{
    INT32 iRet = -1;
...
#if CONFIG_EXFAT_MOUNT_NOFUSE
    snprintf(strMountCmd,sizeof(strMountCmd),"mount %s %s",strDevPath,strMountPath);
#else
    ...
    if(iRet < 0)
    {
        PR_ERR("sys_posix_cmd_call:%s error \n",strMountCmd);
        return iRet;
    }
    return iRet;
}
 ...
 * @return     成果返回OK，失败返回ERROR,
 */
INT32 sys_medium_mount(const CHAR *strDevPath, SYS_MEDIUM_FS_TYPE_E eFsType, const CHAR *strMountPath)
{
    INT32 iRet = -1;
    CHAR strData[128] = {0};
    ...
    {
        PR_ERR("invalid path params dev(%p) mount(%p)\n", strDevPath, strMountPath);
        return iRet;
    }
    ...
    if(iRet < 0)
    {
        PR_ERR("sys_medium_fs_type_convert_string type :%d error \n", eFsType);
        return iRet;
    }
    ...
    if(iRet < 0)
    {
        PR_ERR("convert data type :%d error \n", eFsType);
        return iRet;
    }
...
INT32 sys_medium_make_part(const CHAR *strDevPath, INT32 iPartNum)
{
    ...
    if (iRet != OK)
    {
        PR_ERR("fdisk_make_part %s failed, %s\n", strDevPath, strerror(errno));
        return ERROR;
    }
    return OK;
}
...
INT32 sys_medium_make_mutil_part(const CHAR *strRootDevPath, UINT32 uPartNum, SYS_MEDIUM_PARTITION_INFO_T *pStPartInfo)
{
    ...
    {
        PR_ERR("uPartitionNum support 1 ~ 4, but input %u\n", uPartNum);
        return iRet;
    }
    ...
    if(100u != uRatio)
    {
        PR_ERR("all partition ratio sum not 100!!! uRatio:%u\n",uRatio);
        return iRet;
    }
    iRet = fdisk_make_part_ratio(strRootDevPath, uPartNum, uRatios);
    if (iRet != OK)
    {
        PR_ERR("fdisk_make_part %s failed\n", strRootDevPath);
        return ERROR;
    }
    return OK;
}
 ...
 * @return     成功OK, 其他参考ERROR_CODE_E, 支持get_last_errno获取错误码
 */
INT32 sys_medium_format(const CHAR *strDevPath, SYS_MEDIUM_FS_TYPE_E eFsType, UINT32 uUintSize)
{
    INT32 iRet = -1;
    ...
    if(iRet < 0)
    {
        PR_ERR("strSysFormatCmd %s error \n", strSysFormatCmd);
        return iRet;
    }
    sync();
    return iRet;
 ...
 * @return 成果返回OK，失败返回ERROR
 */
INT32 sys_medium_access_dev_node(const CHAR* strDevNodePath)
{
    INT32 iRet = ERROR;
    if(!strDevNodePath)
    {
 ...
 * @return     成果返回OK，失败返回ERROR,
 */
INT32 sys_medium_check_dev_node(const CHAR *strDevPath)
{
    /*规避措施 后续还是要去掉*/
	INT32 iRet = ERROR;
    INT32 iCnt = 0;
	...
	for(iCnt = 0; iCnt < 2; iCnt++)        /* 当前尝试两次 */
	{
		...
		{
			PR_INF("check dev node:%s successful\n", strDevPath);
			return 0;
		}

		PR_INF("check dev node:%s failed and try again, times = %d\n", strDevPath, iCnt);
 ...
 */
INT32 sys_medium_get_info(const CHAR *strMountPath, SYS_MEDIUM_INFO_T *pStMediumInfo)
{
    INT32 iRet = -1;
    ...
    {
        PR_ERR("invalid params path(%p) info(%p)\n", strMountPath, pStMediumInfo);
        return iRet;
    }
    if (statfs(strMountPath, &stInfo) < 0)
    {
        ...
        PR_ERR("statfs failed - errno[%d:%s]\n", errno, strerror(errno));
        return iRet;
    }
    ...
    switch (stInfo.f_type)
    {
        ...
        case SYS_MEDIUM_FS_MAGIC_EXFAT_NOFUSE:
            /**
             * 当使用标准的exfat文件系统挂载时, f_blocks信息为实际可使用簇大小(剔除了DBR和FAT表), 因此总量差值计算方法为:
             * (总簇数/8)向上取整得到位图占用空间大小, 再根据(位图大小/簇大小)向上取整得到位图占用簇数
             * +2是由于大写字符元文件UpCase和根目录各自占用了一个簇
             */
            uSysBlk += ((stInfo.f_blocks + 7) / 8 + stInfo.f_bsize - 1) / stInfo.f_bsize + 2;
        ...
        case SYS_MEDIUM_FS_MAGIC_EXT4:
            ...
            break;
        case SYS_MEDIUM_FS_MAGIC_YAFFS2:
            pStMediumInfo->eType = SYS_MEDIUM_FS_YAFFS2;
            break;
        default:
            break;
    }
 ...
 */
INT32 sys_medium_is_mounted(const CHAR *pDeviceOrMountpoint, CHAR *pMountpointOrDevice)
{
    FILE *pFp;
    CHAR strLine[128];
    ...
    if (!pDeviceOrMountpoint) {
        PR_ERR("invaild params(null)\n");
        return -EINVAL;
    }

    pFp = fopen("/proc/mounts", "r");
    if (pFp == NULL) {
        PR_ERR("Failed to open /proc/mounts - errno[%d:%s]\n", errno, strerror(errno));
        return ERROR;
    }
...
INT32 sys_medium_get_node_size(const CHAR *strNodePath, UINT32 *puSize)
{
    ...
    if(iRet < 0)
    {
        PR_ERR("call fdisk_get_dev_node_size error\n");
        return ERROR;
    }

    PR_INF("get dev node:%s size:%u\n", strNodePath, *puSize);
...
INT32 sys_medium_get_node_partition(const CHAR *strRootNode, UINT32 *puNum)
{
    ...
    PR_INF("get dev node:%s partition num:%u\n", strRootNode, *puNum);
