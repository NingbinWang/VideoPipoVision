...
INT32 sys_posix_cmd_call(const CHAR *strCmd)
{
    ...
    {
        PR_ERR("socket create unix socket failed - errno[%d:%s]\n", errno, strerror(errno));
        return -1;
    }
    ...
    {
        PR_ERR("bind client unix address %s failed - errno[%d:%s]\n", stClientAddr.sun_path, errno, strerror(errno));
        close(iCliFd);
        return -1;
    }
    if(chmod(stClientAddr.sun_path, S_IRWXU) < 0)
    {
        PR_ERR("chmod client unix address %s failed - errno[%d:%s]\n", stClientAddr.sun_path, errno, strerror(errno));
        close(iCliFd);
        return -1;
    }
    ...
    {
        PR_ERR("unix connect to %s failed - errno[%d:%s]\n", SYS_CMD_SUN_PATH, errno, strerror(errno));
        close(iCliFd);
        return -1;
    }
   ...
   // PR_ERR("%d %d %x \n",stHeader.uCmdLen,strlen(strCmd),strCmd[0]);
    ...
    {
        PR_ERR("recv from execSystemCmd failed - errno[%d:%s]\n", errno, strerror(errno));
        close(iCliFd);
        return -1;
    }
...
INT32 sys_posix_cmd_call(const CHAR *strCmd)
{
    ...
    iRet = system(strCmd);
    if (ERROR == iRet) {
        PR_ERR("system Cmd(%s) failed - errno[%d:%s]\n", strCmd, errno, strerror(errno));
    }
    return iRet;
}
#endif
...
INT32 sys_posix_cmd_call_safety(const CHAR *strCmd)
{
    ...
    if (iStatus != 0) {
        PR_ERR("posix_spawn failed - errno[%d:%s]\n", errno, strerror(errno));
        return ERROR;
    }
    
    /* 3. 等待子进程 */
    if (waitpid(uPid, &iStatus, 0) == -1) {
        PR_ERR("waitpid - errno[%d:%s]\n", errno, strerror(errno));
        return ERROR;
    }
    ...
    if (WIFEXITED(iStatus)) {
        ...
        if (iExitCode != 0) {
            PR_ERR("cmd run failed - exit code(%d)\n", iExitCode);
            return ERROR;
        }
     } else if (WIFSIGNALED(iStatus)) {
        PR_ERR("Child process terminated by signal(%d)\n", WTERMSIG(iStatus));
        return ERROR;
    } else {
        PR_ERR("cmd terminated abnormally\n");
        return ERROR;
    }
...
INT32 sys_posix_mkdir(const CHAR *strPath)
{
    ...
    if (OK != iRet)
    {
        PR_ERR("strPathName:%s failed - errno[%d:%s]\n",strPath, errno, strerror(errno));
    }
    return iRet;
}
                  ...
                  SYS_FD_SET_T *pStExceptionFdSet, SYS_TIMEVAL_T *pStTimeOut)
{
    ...
    if (ERROR == iRet) {
        PR_ERR("select failed - errno[%d:%s]\n", errno, strerror(errno));
    }
    return iRet;
}
...
INT32 sys_posix_processname_get (CHAR *strPname, UINT32 uNameLen)
{
    ...
    if (!strPname || !uNameLen) {
        PR_ERR ("Invaild input params pname %p, len %d\n", 
            strPname, uNameLen);
        return -EINVAL;
    }
    ...
    if (iRet < 0) {
        PR_ERR ("Failed to readlink \"/proc/self/exe\" : %s\n", 
            strerror (errno));
        return -errno;
    }
...
{
    if (!strPname) {
        PR_ERR ("Invaild input params pname %p\n", strPname);
        return -EINVAL;
    }
