 ...
 */
INT32 sockets::socket_create(INT32 iDomain, INT32 iType, INT32 iProtocol)
{
    INT32 iSockFd = -1;

    if(SOCK_AF_INET6 != iDomain && SOCK_AF_INET != iDomain && AF_UNIX != iDomain)
    {
 ...
 * @return     成功返回 套接字句柄 错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sockets::socket_bind(INT32 iSockFd, INT32 iDomain, const CHAR *strIPAddr, UINT16 uPort)
{
    INT32 iRet = -1;
    SOCK_ADDRESS_T stServerAddr;
 ...
 * @return    成功返回 套接字句柄  错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sockets::socket_listen(INT32 iSockFd, INT32 iMaxConNum)
{
 ...
 */
INT32 sockets::socket_accept(INT32 iSockFd, CHAR *strClientIP, UINT16 uAddrLen, UINT16 *puClientPort)
{   
    INT32 iConnFd = -1;
 ...
 */
INT32 sockets::socket_connect(INT32 iSockFd, INT32 iDomain, const CHAR *strIPAddr, UINT16 uPort)
{
    INT32 iRet = ERROR;
    struct sys_address_t stClientAddr;
 ...
 */
INT32 sockets::socket_send_to(INT32 iSockFd, void *pBuf, INT32 iLen, INT32 iDomain, const CHAR *strIPAddr, UINT16 uPort)
{   
    INT32 iRet = ERROR;
    INT32 iSendLen = 0 ;
 ...
 */
INT32 sockets::socket_recvfrom(INT32 iSockFd, VOID *pBuf, INT32 iBufLen, CHAR *strClientIP, UINT16 uAddrLen, UINT16 *puClientPort)
{   
    INT32 iRecvLen = 0;
 ...
 */
INT32 sockets::socket_recv(INT32 iSockFd, VOID *pBuf, INT32 iLen)
{   
    INT32 iRecvNum = -1;
 ...
 * @return     成功发送的字节数 错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sockets::socket_send(INT32 iSockFd, VOID *pBuf, INT32 iLen)
{   
    INT32 iSendLen = 0 ;
 ...
 */
INT32 sockets::socket_close(INT32 iSockFd)
{
    if(close(iSockFd) < 0)
    {
 ...
 */
INT32 sockets::socket_convert_optlevel_and_name(INT32 iOptLevel, INT32 iOptName, INT32 *pConvertOptName, INT32 *pConvertName)
{
    INT32 iRet = ERROR;
 ...
 */
INT32 sockets::socket_set_sock_opt(INT32 iSockFd, INT32 iOptLevel, INT32 iOptName, const VOID *pOptValue, INT32 iOptLength)
{   
    INT32 iRet = ERROR;
 ...
 * @return     成功，返回OK；失败参考ERROR_ID_E, 支持通过get_last_errno获取错误码
 */
INT32 sockets::socket_get_sock_opt(INT32 iSockFd, INT32 iOptLevel, INT32 iOptName, VOID *pOptValue, INT32 *iOptLength)
{
 ...
 */
INT32 sockets::socket_shutdown(INT32 iSockFd, INT32 iHow)
{
 ...
 * @return     成功返回 套接字句柄 错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sockets::socket_get_local_addr(INT32 iSockFd, struct sys_sockaddr *pStLocalAddr, socklen_t *pUAddrLength)
{
    INT32 iRet = ERROR; 
    if(iSockFd < 0)
    {
 ...
 * @return     成功返回 套接字句柄 错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sockets::socket_get_peer_addr(INT32 iSockFd, struct sys_sockaddr *pStRemoteAddr, socklen_t *pUAddrLength)
{
    INT32 iRet = ERROR; 
    if(iSockFd < 0)
    {
 ...
 */
const CHAR *sockets::socket_inet_ntop(INT32 iAf, const VOID *pSrcAddr, CHAR *strDstAddr, socklen_t uSize)
{
 ...
 */
INT32 sockets::socket_inet_pton(INT32 iAf, const VOID *pSrcAddr, VOID *strDstAddr)
{
 ...
 */
INT32 sockets::socket_inet_addr(const CHAR *strIP, ULONG *pUAdddr)
{   
    INT32 iRet = ERROR;
    in_addr_t staddr = -1;
 ...
 */
CHAR *sockets::socket_inet_ntoa(struct sys_in_addr in)
{
 ...
 * @return     成功返回OK 失败返回ERROR
 */
INT32 sockets::socket_inet_aton(const CHAR *strIp, struct sys_in_addr *pStInp)
{
    INT32 iRet = -1 ;
 ...
 */
INT32 sockets::socket_get_tcp_state(int iSockFd)
{
    INT32 iRet = ERROR;
    socklen_t iLen =  0 ;
 ...
 * @return     详见errno 
 */
INT32 sockets::socket_get_last_errno()
{
    return errno;
}