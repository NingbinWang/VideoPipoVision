#ifndef SYS_FILE_INTERFACE_H
#define SYS_FILE_INTERFACE_H
...
/**    
 * @brief      读目录文件(直接读设备方式, 不通过驱动直接获取数据)
 * @param[in]  strDir          查看目录
 * @param[in]  pUserParam      自定义参数
 * @param[in]  pCallBack       读数据回调接口
 * @return     成功 0  失败 -1 
 * @note       1. 直读方式为保证速率申请会一块内存存放fat表和创建部分链表节点不会释放, 需额外调用资源释放接口
 *             2. 直读方式首次调用必须先传入设备节点根目录的直接挂载路径 以此来构建目录链表并读取保存dbr和fat表信息
 */
INT32 sys_file_read_dir_file_direct(const CHAR *strDir, VOID *pUserParam, ReadFileCallBack pCallBack);

/**    
 * @brief      释放直读设备方式申请的内存资源
 * @param      无
 * @return     
 */
VOID sys_file_read_dir_file_direct_free(VOID);
...
/**
 * @brief      关闭文件      
 * @param[in]  iFileID   文件句柄，通过sys_file_open获得
 * @return     成功返回文件句柄 错误返回ERROR，并输出strerror
 */
INT32 sys_file_close(FILE_FD iFileID);
 ...
 */
INT32 sys_file_read(FILE_FD iFileID, VOID *pBuffer, UINT32 uSize);
 ...
 */
INT32 sys_file_write(FILE_FD iFileID, const VOID *pBuffer, UINT32 uSize);
 ...
 */
INT32 sys_file_seek(FILE_FD iFileID, LONG lOffset, INT32 iStartPos);
...
#endif/*__SYS_FILE_INTERFACE_H__*/
