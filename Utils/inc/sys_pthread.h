 ...
 * @param[in]  uStackSize  创建任务栈大小
 * @param[in]  pFunc       创建任务函数
 * @param[in]  uPolicy     调度策略
 * @param[in]  uArgs       参数个数
 * @return     成功返回 0  错误返回 其他
 */
INT32 sys_pthread_create_policy(TASK_ID *pStTid, CHAR *strTaskName, UINT32 uPriority, UINT32 uStackSize,
        FUNCPTR pFunc, UINT8 uPolicy,
        UINT32 uArgs, ...);
 ...
 * @return     成功返回 0  错误返回 其他
 */
INT32 sys_pthread_create_leagcy(TASK_ID *pStTid, CHAR *strTaskName, UINT32 uPriority, UINT32 uStackSize,
        FUNCPTR_L pFunc,
        VOID *pArg);
 ...
 * @return     成功返回 0  错误返回 其他
 */
INT32 sys_pthread_create_leagcy_policy(TASK_ID *pStTid, CHAR *strTaskName, UINT32 uPriority, UINT32 uStackSize,
        FUNCPTR_L pFunc, UINT8 uPolicy,
        VOID *pArg);
 ...
 * @return     成功返回0,错误返回 其他, 参考ERROR_CODE_E
 */
INT32 sys_pthread_set_name( CHAR *strName);
 ...
 * @return     成功返回0,错误返回 其他, 参考ERROR_CODE_E
 */
INT32 sys_pthread_setaffinity(TASK_ID stTaskId, ULONG uCpuset);
 ...
 * @return     成功返回0,错误返回 其他, 参考ERROR_CODE_E
 */
INT32 sys_pthread_getaffinity(TASK_ID stTaskId, ULONG *pCpuset);
 ...
 * @return      无
 */
VOID sys_pthread_set_priority(TASK_ID stTaskId, UINT32 uPriority);

/**
 * @brief       启动bios调度
 * @return      无
 */
VOID sys_pthread_schdule(VOID);
 ...
 *  @return     无
 */
VOID sys_pthread_delete (TASK_ID stTaskId);
 ...
 * @return      无
 */
VOID sys_pthread_sleep(UINT32 uMs);
 ...
 * @return      无
 */
VOID sys_pthread_sleep_us(UINT32 uUs);
 ...
 * @return      无
 */
VOID sys_pthread_suspend_all(VOID);
 ...
 * @return      无
 */
VOID sys_pthread_resume_all(VOID);
 ...
 *              Number of task tick
 */
UINT32 sys_pthread_tick(VOID);

/*
 * @brief      获取线程TID
 * @return     成功返回线程ID, 错误返回0
 */
UINT32 sys_pthread_get_id(VOID);
 ...
 */
INT32 sys_pthread_state(TASK_ID stTaskId, THREAD_STATE_T *pState);
 ...
 */
INT32 sys_pthread_status(THREAD_STATE_T *pStatus);
