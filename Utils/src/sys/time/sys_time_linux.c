 ...
 */
INT32 sys_time_get_msec(UINT64 *puMSecond)
{
    INT32 iRet = ERROR;
 ...
 * @return     成功返回 OK 错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sys_time_get_sec(UINT32 *puSecond)
{
    INT32 iRet = ERROR;
    struct timespec stTimeRun = {0};
    memset(&stTimeRun,0,sizeof(stTimeRun));
 ...
 */
INT32 sys_time_get_utc_sec(UINT32 *puSecond)
{   
    INT32 iRet = ERROR;
 ...
 * @return     成功返回 OK  错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sys_time_get_utc_msec(UINT64 *puUtcMSec)
{   
    INT32 iRet = ERROR;
    struct timeval stTimeVal = {0};
    if(NULL == puUtcMSec)
 ...
 */
INT32 sys_time_convert_tm(const SYS_DATE_TIME_T *pStDateTime,struct tm *pStTime)
{
    INT32 iRet = ERROR;
 ...
 * @return    错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sys_time_set_hw(time_t stTime)
{
    INT32 iRet = ERROR;
    INT32 iRtcFd = -1;
    ...
    if(iRet < 0) 
    {
        PR_ERR("RTC_SET_TIME failed - errno[%d:%s]\n", errno, strerror(errno));
        close(iRtcFd);
        return iRet;
    }
    close(iRtcFd);
 ...
 * @return     成功返回 OK 错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sys_time_set_in_struct(const SYS_DATE_TIME_T *pStDateTime)
{   
#ifdef ANDROID
    PR_ERR("no support stime in andriod \n");
    return ERROR;
#else
    INT32 iRet = ERROR;
    struct tm stTime= {0};
    ...
    if (-1 == stCurrentTime) {
        PR_ERR("mktime set (%04u-%02u-%02u %02u:%02u:%02u)(%ld) failed - errno[%d:%s]\n",
            pStDateTime->uYear, pStDateTime->uMonth, pStDateTime->uDay, pStDateTime->uHour, pStDateTime->uMinute, pStDateTime->uSec,
            stCurrentTime, errno, strerror(errno));
        return ERROR;
    } else if (stCurrentTime < -1) {
        PR_WRN("mktime set (%04u-%02u-%02u %02u:%02u:%02u)(%ld) abnormal\n",
            pStDateTime->uYear, pStDateTime->uMonth, pStDateTime->uDay, pStDateTime->uHour, pStDateTime->uMinute, pStDateTime->uSec,
            stCurrentTime);
    }
#if __GLIBC_MINOR__ >= 31
 ...
 * @return     成功返回 OK 错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sys_time_get_in_struct(SYS_DATE_TIME_T *pStDateTime)
{
    time_t now;
    struct tm stTimeNow;
    if (pStDateTime == NULL)
    ...
    {
        PR_ERR("get current time failed - errno[%d:%s]\n", errno, strerror(errno));
        return ERROR;
    }
 ...
 * @return     成功返回 OK 错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sys_time_convert_struct_to_value(const SYS_DATE_TIME_T *pStDateTime,UINT32 *uUtcTime)
{   
    struct tm stTime = {0};
    time_t stTimeSec = 0;
    if(NULL == pStDateTime || uUtcTime == NULL)
    {
        PR_ERR("param  is invaild \n");
        return ERROR;
    }
    ...
    if (-1 == stTimeSec) {
        PR_ERR("mktime convert (%04u-%02u-%02u %02u:%02u:%02u)(%ld) failed - errno[%d:%s]\n",
            pStDateTime->uYear, pStDateTime->uMonth, pStDateTime->uDay, pStDateTime->uHour, pStDateTime->uMinute, pStDateTime->uSec,
            stTimeSec, errno, strerror(errno));
        return ERROR;
    } else if (stTimeSec < -1) {
        PR_WRN("mktime convert (%04u-%02u-%02u %02u:%02u:%02u)(%ld) abnormal\n",
            pStDateTime->uYear, pStDateTime->uMonth, pStDateTime->uDay, pStDateTime->uHour, pStDateTime->uMinute, pStDateTime->uSec,
            stTimeSec);
    }
 ...
 * @return     成功返回 OK   错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sys_time_convert_value_to_struct(const UINT32 uUtcTime, SYS_DATE_TIME_T *pStDateTime)
{
    struct tm stTimeNow;
    time_t uTimeTemp = 0;
    ...
    {
        PR_ERR("localtime_r failed - errno[%d:%s]\n", errno, strerror(errno));
        return ERROR;
    }
...
/**@fn         sys_time_set_utc_msec
 * @brief      设置系统当前UTC时间
 * @param[in]  uUtcMSec       UTC时间ms
 * @return     成功返回 ok  错误返回 ERROR, 支持通过get_last_errno获取错误码
 */
INT32 sys_time_set_utc_msec(UINT64 uUtcMSec)
{
    INT32 iRet = 0;
    ...
    if (iRet < 0) {
        PR_ERR ("Failed to setup system clock (clock_settime -> realtime) : %s\n",
            strerror (errno));
        return iRet;
    }
#else
    ...
    iRet = stime(&stTime);
    if (iRet < 0) {
        PR_ERR ("Failed to setup system clock (stime) : %s\n",
            strerror (errno));
        return iRet;
    }
#endif
 ...
 */
INT32 sys_time_convert_value_to_str(UINT32 uTime, CHAR *strData, UINT32 uSize)
{
 ...
 * @return     成功返回OK，失败返回ERROR
 */
INT32 sys_time_convert_str_to_value(const CHAR *strData, UINT32 *uTime)
{   
    INT32 iRet = ERROR;
    ...
    {
        PR_ERR("time str process failed - errno[%d:%s]\n", errno, strerror(errno));
        return iRet;
    }
    ...
    if(iRet < 0)
    {
        PR_ERR("date time convert error!\n");
        return ERROR;
    }
    *uTime = uTimeValue;
    return OK;
 ...
 * @return     成功返回OK，失败返回ERROR
 */
INT32 sys_time_convert_str_to_datetime(const CHAR *strData, SYS_DATE_TIME_T *pStDateTime)
{
    ...
    if (strlen(strData) > 20) {
        if(sscanf(strData, "%04d-%02d-%02d %02d:%02d:%02d,%03d", &uYear, &uMon, &uDay, &uHour, &uMin, &uSec, &uMilleSecond) != 7) {
            PR_ERR("sscanf(%s)(7) failed - errno[%d:%s]\n", strData, errno, strerror(errno));
            return ERROR;
        }
    } else {
        if(sscanf(strData, "%04d-%02d-%02d %02d:%02d:%02d", &uYear, &uMon, &uDay, &uHour, &uMin, &uSec) != 6) {
            PR_ERR("sscanf(%s)(6) failed - errno[%d:%s]\n", strData, errno, strerror(errno));
            return ERROR;
        }
    }
 ...
 * @return     成功返回OK，失败返回ERROR
 */    
INT32 sys_time_convert_struct_to_str(const SYS_DATE_TIME_T *pStDateTime, CHAR *strData, UINT16 uBufLen)
{
    ...
    {
        PR_ERR("snprintf failed - errno[%d:%s]\n", errno, strerror(errno));
        return ERROR;
    }
 ...
 * @return     成功返回OK，失败返回ERROR
 */    
INT32 sys_time_str_format_time(const SYS_DATE_TIME_T *pStDateTime, const CHAR *strFromat, CHAR *strBuf,UINT16 uBufSize)
{
    INT32 iRet = ERROR;
    struct tm stTime = {0};
 ...
 * @return     成功返回OK，失败返回ERROR
 */    
INT32 sys_time_sleep_second(UINT32 uSecond)
{
    INT32 iRet =  ERROR;
    iRet = sleep(uSecond);
    return iRet;
 ...
 * @return     成功返回OK，失败返回ERROR
 */    
INT32 sys_time_sleep_ms(UINT64 uMillSecond)
{
    INT32 iRet =  ERROR;
 ...
 * @return     成功返回OK，失败返回ERROR
 */    
INT32 sys_time_sleep_us(UINT32 uMicroSecond )
{
    INT32 iRet =  ERROR;
