...
INT32 sys_share_memeory_get(INT32 iKey, UINT64 uSize, SYS_SHM_MODE_BIT_E eIpcType)
{
    ...
    if(iShmID < 0)
    {
        PR_ERR("iKey:%u shmget error,ERROR:%s\n",iKey,strerror(errno));
        return iRet;
    }
    return iShmID;
}
...
VOID *sys_share_memeory_attach(INT32 iShmID, const VOID *pShareMemoryAddr, SYS_SHM_ATTACH_FLAG_E eAttachFlag)
{
    ...
    if(MAP_FAILED == pBuf)
    {
        PR_ERR("iShmID:%d shmat error,ERROR:%s\n",iShmID,strerror(errno));
        return NULL;
    }
    return pBuf;
}
...
INT32 sys_share_memeory_control(INT32 iShmID, SYS_SHM_CMD_IPC_CMD_E eCmd, SYS_SHM_INFO_T *pStData)
{
    ...
    if(iRet < 0)
    {
        PR_ERR("shmctl error,ERROR:%s\n",strerror(errno));
        return ERROR;
    }
    return OK;
}

/**@fn         sys_share_memeory_open      
 * @brief      posix共享内存创建接口
...
INT32 sys_share_memeory_unlink(const CHAR *strName)
{
    ...
    if(iRet < 0)
    {
        PR_ERR("shm_unlink error,ERROR:%s\n",strerror(errno));
        return ERROR;
    }
