...
{   
    ...
    {
        PR_ERR("create socket failed - errno[%d:%s]\n", errno, strerror(errno));
        return ERROR;
    }
    ...
    if(ioctl(iFd, SIOCSIFADDR, &stIfreq) < 0)
   	{
		PR_ERR("IP_set ioctl failed. error: %s!\n", strerror(errno));
		close(iFd);
		return ERROR;
	}
...
INT32 sys_net_get_ip(const CHAR* strEthName, INT32 iDomain, CHAR *strIpAddr, UINT16 uLength)
{
    ...
    {
        PR_ERR("get_IP:%s : ioctl failed! error: %s\n",strEthName,strerror(errno));
        close(iFd);
        return ERROR;
	}
...
INT32 sys_net_set_mac_addr(const CHAR* strEthName,UINT8 *pStMacAddr,UINT16 uLength)
{
    ...
    {
        PR_ERR("socket failed - errno[%d:%s]\n", errno, strerror(errno));
        return iRet;
    }

    if (ioctl(iFd, SIOCGIFHWADDR, &stIfreq) == -1) {
        PR_ERR("ioctl get hwaddr - errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return iRet;
    }
    ...
    if(iRet < 0)
    {
        PR_ERR("ioctl set hwaddr faield - errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return iRet;
    }
...
INT32 sys_net_get_mac_addr(const CHAR* strEthName,UINT8 *pStMacAddr,UINT16 uLength)
{
    ...
    if(iRet < 0)
    {
        PR_ERR("ioctl get hwaddr failed - errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return iRet;
    }
    memcpy(pStMacAddr, stIfreq.ifr_hwaddr.sa_data, min(uLength, sizeof(stIfreq.ifr_hwaddr.sa_data)));
    close(iFd);
    return iRet;
}
...
INT32 sys_net_set_netmask(const CHAR* strEthName,UINT8 *pStAddr,UINT16 uLength)
{
    ...
    if (ioctl(iFd, SIOCGIFNETMASK, &stIfreq) == -1) {
        PR_ERR("ioctl get netmask - errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return iRet;
    }
    ...
    if(iRet < 0)
    {
        PR_ERR("ioctl set netmask failed - errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return iRet;
    }
    close(iFd);
...
INT32 sys_net_get_netmask(const CHAR* strEthName,UINT8 *pStAddr,UINT16 uLength)
{
    ...
    if(iRet < 0)
    {
        PR_ERR("ioctl get netmask failed - errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return iRet;
    }
    pAddr = (struct sockaddr_in *)&stIfreq.ifr_netmask;
...
INT32 sys_net_set_gateway(const CHAR* strEthName,UINT8 *pStAddr,UINT16 uLength)
{
    ...
    if (iRet < 0) {
        PR_ERR("ioctl set gateway failed - errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return iRet;
    }
...
INT32 sys_net_get_gateway(const CHAR* strEthName,UINT8 *pStAddr,UINT16 uLength)
{
    ...
    if (!pStAddr || !strEthName) {
        PR_ERR("Invalid pStAddr(%p) strEthName(%p)\n", pStAddr, strEthName);
        return ERROR;
    }

    pFp = fopen ("/proc/net/route", "r");
    if (NULL == pFp) {
        PR_ERR ("can't open /proc/net/route - errno[%d:%s]\n", errno, strerror(errno));
        return ERROR;
    }
...
INT32 sys_net_dev_node_check(const CHAR *strDevName)
{
    ...
    {
        PR_ERR("ioctl get flags failed. dev may not exist - errno[%d:%s]\n", errno, strerror(errno));
        close(iSocketFd);
        return ERROR;
    }
...
INT32 sys_net_ifconfig_up(const CHAR *strName)
{
    ...
    if ( (iRtn = ioctl(iFd, SIOCSIFFLAGS, &ifr) ) != 0) {
        PR_ERR("ioctl set flags failed - errno[%d:%s]\n", errno, strerror(errno));
    }
...
INT32 sys_net_ifconfig_down(const CHAR *strName)
{
    ...
    if ( (iRtn = ioctl(iFd, SIOCSIFFLAGS, &ifr) ) != 0) 
    {
        PR_ERR("ioctl set failed - errno[%d:%s]\n", errno, strerror(errno));
    }
...
INT32 sys_net_ifconfig(const CHAR *strDevName, const CHAR *strIP,const CHAR *strNetMask, const CHAR *strGateWay)
{
    ...
    {
        PR_ERR("inet_aton failed errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return ERROR;
    }
    ...
    {
        PR_ERR("ioctl(SIOCADDRT) error in set_default_route - errno[%d:%s]\n", errno, strerror(errno));
        close(iFd);
        return ERROR;
    }
    close(iFd);

    PR_INF("== set OK!!! if: %s, ip: %s mask: %s ip: %s==\n", strDevName,strIP,strNetMask,strGateWay);
    return OK;
}
 ...
 */
INT32 sys_net_get_eth_list(CHAR **astrEhtName, UINT8 uNameSize, UINT8 uNumMax)
{
    ...
    struct ifconf ifconf = {
        .ifc_len = sizeof(strBuffer),
        .ifc_buf = strBuffer,
    };
    //获取所有网络接口信息
    if (ioctl(iSock, SIOCGIFCONF, &ifconf) < 0) {
        PR_ERR("ioctl get ifconf failed - errno[%d:%s]\n", errno, strerror(errno));
        close(iSock);
        return ERROR;
    }

    uEthNum = ifconf.ifc_len / sizeof(struct ifreq);
    pIfr = (struct ifreq *)strBuffer;
    for (uCnt = 0; uCnt < uEthNum && uCnt < uNumMax; uCnt++) {
        strcpy(pName + uNameSize * uCnt, pIfr[uCnt].ifr_name);
    }
