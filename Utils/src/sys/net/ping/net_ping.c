...
static INT32 icmp_msg_recv(INT32 iSockFd, VOID *pBuffer, UINT32 uLen, UINT32 uWaitTime)
{
    ...
    if(WAIT_FOREVER == uWaitTime)
    {
        ...
        if (0 == uWaitTime) {
            PR_WRN ("Timeout cannot be 0, use default(2000ms)\n");
            uWaitTime = 2000;
        }
    ...
    iRet = select(iSockFd + 1, &stReadSets, NULL, &stErrSets, pWaitTime);
     if (iRet > 0 && FD_ISSET(iSockFd, &stReadSets))
     {
         ...
         if (iRet < 0) {
             PR_ERR ("recv failed, ret(%d) errno[%d]: %s\n", iRet, errno, strerror(errno));
             return -errno;
         }
     } else {
        PR_ERR ("select failed, ret(%d) errno[%d]: %s\n", iRet, errno, strerror(errno));
        return -errno;
     }
 ...
 */
INT32 sys_net_ping(CHAR *pDstIpAddr, UINT32 *pTimeout, UINT32 *pTtl)
{
    ...
    if (iSockFd < 0) {
        PR_ERR ("Failed to create socket errno[%d]: %s\n", errno, strerror(errno));
        return errno;
    }
    ...
    PR_DBG("iphdr(%ld) icmphdr(%ld) datalen(%ld) totlen(%d)\n", 
    ...
    if (sendto(iSockFd, &stIcmpPingMsg, sizeof(stIcmpPingMsg), 0, (struct sockaddr*)&stDestAddr, sizeof(stDestAddr)) < 0) {
        PR_ERR ("sendto icmp msg failed,errno[%d]: %s\n", errno, strerror(errno));
        close(iSockFd);
        return -errno;
    }
    ...
    *pTtl = stIcmpPingMsg.stIpHdr.ttl;
    PR_DBG ("%d bytes from %s:icmp_seq=%u ttl=%d rtt=%.3f ms\n",
