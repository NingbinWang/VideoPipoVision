...
int post_process(RknnConText_T *app_ctx, void *outputs, RESULT_GROUP_T *od_results)
{
    ...
    {  
         ...
         if (app_ctx->is_quant)
        {
            validCount += process_i8((int8_t *)_outputs[i].buf, (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride, filterBoxes, objProbs,
                                     classId, app_ctx->box_conf_threshold, app_ctx->output_attrs[i].zp, app_ctx->output_attrs[i].scale);
        }
        else
        {
            validCount += process_fp32((float *)_outputs[i].buf, (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride, filterBoxes, objProbs,
                                       classId,  app_ctx->box_conf_threshold);
        }
    }
    ...
    {
        nms(validCount, filterBoxes, classId, indexArray, c, app_ctx->nms_threshold);
    }
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        float x1 = filterBoxes[n * 4 + 0] - app_ctx->x_pad;
        float y1 = filterBoxes[n * 4 + 1] - app_ctx->y_pad;
        float x2 = x1 + filterBoxes[n * 4 + 2];
        float y2 = y1 + filterBoxes[n * 4 + 3];
        int id = classId[n];
        float obj_conf = objProbs[i];

        od_results->results[last_count].box.left = (int)(clamp(x1, 0, model_in_w) / app_ctx->scale_w);
        od_results->results[last_count].box.top = (int)(clamp(y1, 0, model_in_h) / app_ctx->scale_h);
        od_results->results[last_count].box.right = (int)(clamp(x2, 0, model_in_w) / app_ctx->scale_w);
        od_results->results[last_count].box.bottom = (int)(clamp(y2, 0, model_in_h) / app_ctx->scale_h);
