...
MppDecoder::MppDecoder(int type, int fps, void* usrdata, DecCallback callback)
{
    Init(type, fps);
...
int MppDecoder::Init(int v_type, int fps)
{
    MPP_RET ret         = MPP_OK;
    this->fps = fps;
    ...
    if(v_type == 264) {
        ...
        LOG_DEBUG("unsupport video_type %d",v_type);
        return -1;
    }
    LOG_DEBUG("mpi_dec_test start ");
    memset(&loop_data, 0, sizeof(loop_data));
    LOG_DEBUG("mpi_dec_test decoder test start mpp_type %d ",mpp_type);
    ...
    if (ret) {
        LOG_DEBUG("%p mpp_init failed ", mpp_ctx);
        return -1;
    }
    ...
    if (ret) {
        LOG_DEBUG("%p failed to get decoder cfg ret %d ", mpp_ctx, ret);
        return -1;
    }
    ...
    if (ret) {
        LOG_DEBUG("%p failed to set split_parse ret %d ", mpp_ctx, ret);
        return -1;
    }

    ret = mpp_mpi->control(mpp_ctx, MPP_DEC_SET_CFG, cfg);
    if (ret) {
        LOG_DEBUG("%p failed to set cfg %p ret %d ", mpp_ctx, cfg, ret);
        return -1;
    }
...
int MppDecoder::Decode(uint8_t* pkt_data, int pkt_size, int pkt_eos)
{
    ...
    LOG_DEBUG("receive packet size=%d ", pkt_size);
    ...
    do {
        ...
        do {
            ...
            if (MPP_OK != ret) {
                LOG_DEBUG("decode_get_frame failed ret %d ", ret);
                break;
            }
            ...
            if (frame) {
                ...
                LOG_DEBUG("decoder require buffer w:h [%d:%d] stride [%d:%d] buf_size %d pts=%lld dts=%lld ",
                ...
                if (mpp_frame_get_info_change(frame)) {
                    ...
                    // if (ret) {
                    //     LOG_DEBUG("get mpp buffer group  failed ret %d ", ret);
                    //     break;
                    // }
                    ...
                    if (NULL == data->frm_grp) {
                        ...
                        ret = mpp_buffer_group_get_internal(&data->frm_grp, MPP_BUFFER_TYPE_DRM);
                        if (ret) {
                            LOG_DEBUG("%p get mpp buffer group failed ret %d ", ctx, ret);
                            break;
                        }
                        ...
                        if (ret) {
                            LOG_DEBUG("%p set buffer group failed ret %d ", ctx, ret);
                            break;
                        }
                    } else {
                        ...
                        if (ret) {
                            LOG_DEBUG("%p clear buffer group failed ret %d ", ctx, ret);
                            break;
                        }
                    }
                    ...
                    if (ret) {
                        LOG_DEBUG("%p limit buffer group failed ret %d ", ctx, ret);
                        break;
                    }
                    ...
                    if (ret) {
                        LOG_DEBUG("%p info change ready failed ret %d ", ctx, ret);
                        break;
                    }
                    ...
                    struct timeval tv;
                    gettimeofday(&tv, NULL);
                    LOG_DEBUG("get one frame %ld ",  (tv.tv_sec * 1000 + tv.tv_usec/1000));
                    ...
                    LOG_DEBUG("time_gap=%ld", time_gap);
                    if (time_gap > 0) {
        ...
        if (data->frame_num > 0 && data->frame_count >= data->frame_num) {
            data->eos = 1;
            LOG_DEBUG("reach max frame number %d ", data->frame_count);
            break;
        }
...
int MppDecoder::SetCallback(DecCallback callback) {
